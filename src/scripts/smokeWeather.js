...
async function main() {
  ...
  try {
    ...
    const plantResp = await fetch(`${base}/plants`, {
      ...
      body: JSON.stringify({
        name: `Smoke Plant ${Date.now()}`,
        type: 'solar',
        latitude: 23.241999,
        longitude: 69.666932,
        capacityMW: 50,
        hasLimitedHistory: true,
        solarSpec: { panelTiltDeg: 20, panelAzimuthDeg: 180 },
      }),
    });
    ...
    const weatherResp = await fetch(`${base}/plants/${plantId}/weather?hours=24`, {
      headers: { Authorization: `Bearer ${token}` },
    });
    const weatherJson = await weatherResp.json().catch(() => null);
    if (!weatherResp.ok) {
      throw new Error(`Weather failed: ${weatherResp.status} ${JSON.stringify(weatherJson)}`);
    }
    ...
    console.log({
      ...
      hourlyPoints: Array.isArray(weatherJson.hourly) ? weatherJson.hourly.length : null,
      firstHour: weatherJson.hourly?.[0]?.time,
    });
  } finally {

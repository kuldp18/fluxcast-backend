...
async function main() {
  ...
  try {
    ...
    const plantResp = await fetch(`${base}/plants`, {
      ...
      body: JSON.stringify({
        name: `Forecast Smoke Plant ${Date.now()}`,
        type: 'solar',
        latitude: 23.241999,
        longitude: 69.666932,
        capacityMW: 50,
    ...
    const trigger = await fetch(`${base}/plants/${plantId}/forecast`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
      },
      body: JSON.stringify({ horizon: 24 }),
    });
    const triggerJson = await trigger.json().catch(() => null);
    // eslint-disable-next-line no-console
    console.log('trigger', trigger.status, triggerJson);

    // GET remains 404 until a real workflow/worker writes ForecastResult.
